...
void bugo_ecs_init_camera2d(ecs_world_t* world)
{
    ...
    ecs_set_id(bugo_ecs_world(), ecs_camera2d_t, ecs_camera2d_t, sizeof(Camera2D), 
    &(Camera2D){.zoom = 1.0f, .rotation = 0.0f, .target = bugo_math_vector2_one(), .offset = bugo_math_vector2_one()});
}
