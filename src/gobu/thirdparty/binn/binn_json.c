...
static void json_foreach_array(JsonObject *object, guint index_, JsonNode *node, binn *list)
{
...
binn *APIENTRY json_obj_to_binn(JsonNode *node)
{
  ...
  case JSON_NODE_VALUE:
    ...
    {
    case G_TYPE_STRING:
      return binn_string(json_node_get_string(node), BINN_TRANSIENT);
      break;
    case G_TYPE_BOOLEAN:
...
binn *APIENTRY binn_serialize_load(char *str)
{
  ...
  if (json_parser_load_from_data(parser, str, strlen(str), NULL))
  {
    ...
    return json_obj_to_binn(reader);
  }
  return NULL;
}
