...
static void predraw_begin_rendering(ecs_iter_t *it)
{
    ...
    for (int i = 0; i < it->count; i++)
    {
        gb_gfx_begin_drawing();
        gb_gfx_clear_background(RF_BLACK);
        gb_gfx_begin_camera2d(camera[i]);

        if (win[i].show_grid)
            gb_gfx_draw_grid_2d(win[i].width, 48, (gb_color_t){20, 20, 20, 255});
    }
}
