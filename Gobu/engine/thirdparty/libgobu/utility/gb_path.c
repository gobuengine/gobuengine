...
{
    return gb_fs_test(pathname, G_FILE_TEST_IS_DIR);
}
...
{
    return g_mkdir_with_parents(pathname, 0777) == -1 ? false : true;
}
...
{
    return gb_path_normalize(gb_path_join(project_path, FOLDER_CONTENT_PROJECT, path, NULL));
}
...
{
    return gb_fs_test(filename, G_FILE_TEST_IS_DIR);
}
