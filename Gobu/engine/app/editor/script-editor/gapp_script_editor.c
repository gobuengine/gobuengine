...
GtkSourceLanguage *gapp_script_editor_get_language(void)
{
    GtkSourceLanguageManager *managerLanguage = gtk_source_language_manager_get_default();
    return gtk_source_language_manager_get_language(managerLanguage, GAPP_SCRIPT_LANGUAGE_DEFAULT);
}

GtkSourceStyleScheme *gapp_script_editor_get_style_scheme(void)
{
    GKeyFile *config = gapp_config_load_from_file("editor.config");
    gchar *script = gapp_config_editor_theme_script(config);

    GtkSourceStyleSchemeManager *scheme_manager = gtk_source_style_scheme_manager_get_default();
    GtkSourceStyleScheme *scheme = gtk_source_style_scheme_manager_get_scheme(scheme_manager, script);
    ...
    gtk_box_append(self, vbox);
    {
...

        {
            ...
            // gtk_widget_set_size_request(GTK_WIDGET(view_source_scroll), -1, -1);
            {
                ...
                gtk_source_buffer_set_style_scheme(self->buffer, gapp_script_editor_get_style_scheme());
                gtk_source_buffer_set_language(self->buffer, gapp_script_editor_get_language(GAPP_SCRIPT_LANGUAGE_DEFAULT));
